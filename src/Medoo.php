...
class Medoo
{
    ...
    {
        return $this->exec($this->buildRaw($this->raw($query, $map), $map), $map);
    }
    ...
    protected function buildRaw($raw, &$map)
    {
        ...
        $query = preg_replace_callback(
            ...
            function ($m) {
                return !empty($ma[2]) ? $m[2] . ' ' . $this->tableQuote($m[3]) : $this->columnQuote($m[3]);
            },
            $raw->value
        );
    ...
    protected function columnPush(&$columns, &$map)
    {
        ...
        foreach ($columns as $key => $value) {
            if (is_array($value)) {
                ...
                preg_match('/(?<column>[a-zA-Z0-9_\.]+)(?:\s*\((?<alias>[a-zA-Z0-9_]+)\))?'
                    . '(?:\s*\[(?<type>(?:String|Bool|Int|Number|Object|JSON))\])?/i', $value, $match);
    ...
    protected function dataImplode($data, &$map, $conjunctor)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($type === 'array') {
                $relationship = strpos($key, 'AND', 0) !== false
                    ? 'AND'
            ...
            if (is_int($key) && preg_match(
                ...
                $stack[] = $this->columnQuote($match[1]) . ' ' . $match['operator'] . ' '
                    . $this->columnQuote($match[3]);
            } else {
                preg_match(
                ...
                if (isset($match['operator'])) {
                    ...
                    if (in_array($operator, ['>', '>=', '<', '<='])) {
                        ...
                        if ($type === 'array') {
                            ...
                            $data_type = is_numeric($value[0]) && is_numeric($value[1])
                                ? PDO::PARAM_INT
                                : PDO::PARAM_STR;
            ...
            if (isset($where['MATCH'])) {
                ...
                if (is_array($MATCH) && isset($MATCH['columns'], $MATCH['keyword'])) {
                    ...
                    $where_clause .= ($where_clause !== '' ? ' AND ' : ' WHERE')
                        . ' MATCH (' . $columns . ') AGAINST (' . $map_key . $mode . ')';
                }
            }
    ...
    protected function columnMap($columns, &$stack)
    {
        ...
        foreach ($columns as $key => $value) {
            if (is_int($key)) {
                preg_match('/([a-zA-Z0-9_]+\.)?(?<column>[a-zA-Z0-9_]+)(?:\s*\((?<alias>[a-zA-Z0-9_]+)\))?'
                    . '(?:\s*\[(?<type>(?:String|Bool|Int|Number|Object|JSON))\])?/i', $value, $key_match);
    ...
    public function insert($table, $datas)
    {
        ...
        return $this->exec('INSERT INTO ' . $this->tableQuote($table) . ' (' . implode(', ', $fields)
    ...
    public function update($table, $data, $where = null)
    {
        ...
        return $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $fields)
    ...
    public function replace($table, $columns, $where = null)
    {
        ...
        foreach ($columns as $column => $replacements) {
            if (is_array($replacements)) {
                foreach ($replacements as $old => $new) {
                    ...
                    $stack[] = $this->columnQuote($column) . ' = REPLACE(' . $this->columnQuote($column)
                        . ', ' . $map_key . 'a, ' . $map_key . 'b)';
        ...
        if (!empty($stack)) {
            return $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $stack)
                . $this->whereClause($where, $map), $map);
        }
    ...
    public function has($table, $join, $where = null)
    {
        ...
        $query = $this->exec('SELECT EXISTS('
            . $this->selectContext($table, $map, $join, $column, $where, 1) . ')', $map);
